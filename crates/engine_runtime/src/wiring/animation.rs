//! Game-level animation wiring from ECS semantics into locomotion states.
...
use engine_world::components::{AiState, Goal, LifeStage};
...
impl LegacyEngineSystem for AnimationIntegrationSystem {
    ...
    fn descriptor(&self) -> SystemDescriptor {
        SystemDescriptor::new("AnimationIntegration").reads_event::<BodyZoneDamaged>()
    }
    ...
    fn fixed_tick(&mut self, ctx: &mut FixedTickContext) {
        ...
        for &entity in &ctx.ecs.alive {
            ...
            let machine = self
                .locomotion
                .entry(entity)
                .or_insert_with(LocomotionMachine::new);
            machine.update(speed, is_dead, SIM_DT);
        }
