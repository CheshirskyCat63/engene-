...
// Add winit 0.29 compatible types  
use winit::event::{DeviceEvent, DeviceId, ElementState, KeyEvent, MouseButton, WindowEvent};

// Stubs for winit 0.29 compatibility
pub trait ApplicationHandler<T = ()> {
    fn resumed(&mut self, _event_loop: &EventLoop<T>);
    fn window_event(&mut self, _event_loop: &EventLoop<T>, _window_id: winit::window::WindowId, _event: WindowEvent);
    fn device_event(&mut self, _event_loop: &EventLoop<T>, _device_id: DeviceId, _event: DeviceEvent);
    fn about_to_wait(&mut self, _event_loop: &EventLoop<T>);
}
pub type ActiveEventLoop<T = ()> = EventLoop<T>;
...
pub struct Ecs;
impl Ecs {
    pub fn get_kind(&self, _entity: u32) -> Option<EntityKind> {
        Some(EntityKind::Npc)
    }
}
