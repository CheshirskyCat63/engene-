use crate::prefabs::prefab_registry::PrefabRegistry;
    ...
    for name in registry.names() {
        ...
        if prefab.root.components.is_empty() {
            warnings.push(format!(
                "Prefab '{}' has no components on root entity",
                name
            ));
        }
        ...
        if let Some(ref base) = prefab.base {
            if registry.get(base).is_none() {
                errors.push(format!(
                    "Prefab '{}' inherits from '{}' which doesn't exist",
                    name, base
                ));
            }
        }
        ...
        if entity_count > 100 {
            warnings.push(format!(
                "Prefab '{}' has {} entities (consider splitting)",
                name, entity_count
            ));
        }
    }
