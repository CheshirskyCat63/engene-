use crate::system_descriptor::{DeterminismTier, SystemDescriptor};
        ...
        for issue in &self.issues {
            match issue {
                ValidationIssue::WriteWriteConflict {
                    ...
                    println!(
                        "[VALIDATION ERROR] Write-write conflict: '{}' <-> '{}': {}",
                        system_a, system_b, description
                    );
                }
                ValidationIssue::WriteReadHazard {
                    writer,
                    reader,
                    description,
                } => {
                    println!(
                        "[VALIDATION WARN] Write-read hazard: '{}' writes, '{}' reads: {}",
                        writer, reader, description
                    );
                }
                ValidationIssue::HeadlessUnsafe { system } => {
                    println!(
                        "[VALIDATION WARN] System '{}' is not headless-compatible",
                        system
                    );
                }
                ValidationIssue::DeterminismViolation {
                    ...
                    println!(
                        "[VALIDATION ERROR] Determinism violation in '{}': {}",
                        system, description
                    );
                }
            }
