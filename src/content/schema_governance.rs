...
impl SchemaRegistry {
    ...
    {
        self.migrations.insert((schema.to_string(), from, to), Box::new(migrate));
    }
    ...
    pub fn migrate(&self, schema: &str, from: u32, to: u32, data: &[u8]) -> Option<Vec<u8>> {
        ...
        if from < to {
            ...
            while current_version < to {
                ...
                if let Some(migrator) = self.migrations.get(&(schema.to_string(), current_version, next_version)) {
...
impl ContentMetadata {
    pub fn from_file(path: impl Into<PathBuf>, schema_name: impl Into<String>) -> std::io::Result<Self> {
        let path = path.into();
...
fn extract_version_from_file(path: &Path) -> Option<u32> {
    ...
    for line in header_str.lines().take(5) {
        if line.starts_with("version:") || line.starts_with("schema_version:") {
            let version_str: String = line.split(':').nth(1)?.chars().filter(|c| c.is_ascii_digit()).collect();
...
impl ContentValidator {
    ...
    pub fn validate_file(&self, path: &Path, schema_name: &str) -> ValidationResult {
        // Check file exists
        if !path.exists() {
            return ValidationResult::fail(path.to_path_buf(), format!("File not found: {}", path.display()));
        }

        // Get schema info
        let schema = match self.registry.get(schema_name) {
            Some(s) => s,
            None => return ValidationResult::fail(path.to_path_buf(), format!("Unknown schema: {}", schema_name)),
        };
        ...
        if schema.deprecated {
            result.warnings.push(format!(
                ...
                schema.name,
                schema.deprecation_message.as_deref().unwrap_or("no message")
            ));
        }
    ...
    pub fn validate_directory(&self, dir: &Path) -> Vec<ValidationResult> {
        ...
        for entry in walkdir::WalkDir::new(dir).into_iter().filter_map(|e| e.ok()) {
            ...
            let schema_name = path.file_name()
                .and_then(|n| n.to_str())
                .and_then(|name| {
                    schema_map.iter()
...
pub fn generate_governance_report(results: &[ValidationResult]) -> String {
    ...
    if invalid > 0 {
        ...
        for result in results.iter().filter(|r| !r.valid) {
            report.push_str(&format!("### {} (v{})\n", result.path.display(), result.schema_version));
