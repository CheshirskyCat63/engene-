...
impl TypedChannel {
    fn new(capacity: usize) -> Self {
        Self { events: Vec::new(), capacity, dropped_count: 0, total_emitted: 0 }
    }
...
impl EventBus {
    ...
    pub fn set_channel_capacity<E: 'static>(&mut self, capacity: usize) {
        let channel = self.channels.entry(TypeId::of::<E>())
    ...
    pub fn emit_boxed(&mut self, event: Box<dyn Any + Send + Sync>) {
        ...
        let cap = self.default_capacity;
        let channel = self.channels.entry(type_id)
            .or_insert_with(|| TypedChannel::new(cap));
        channel.push(event);
    }
    ...
    pub fn read<E: 'static>(&self) -> Vec<&E> {
        self.channels
            .get(&TypeId::of::<E>())
            .map(|ch| ch.events.iter().filter_map(|e| e.downcast_ref::<E>()).collect())
            .unwrap_or_default()
    }
    ...
    pub fn get_sticky<E: 'static>(&self) -> Option<&E> {
        self.sticky.get(&TypeId::of::<E>()).and_then(|e| e.downcast_ref::<E>())
    }
