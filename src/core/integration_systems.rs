...
use crate::ai::combat::StaggerState;
use crate::ai::combat_tactics::cover::{evaluate_cover_at, find_nearby_cover, CoverPoint};
use crate::ai::combat_tactics::coordination::{assign_group_roles, find_group_members, GroupRole};
use crate::ai::combat_tactics::tactics::{select_tactic, Tactic, TacticProfile};
use crate::ai::combat_tactics::threat::{assess_threats, ThreatEntry};
use crate::ai::decision::{decide_monster, decide_npc};
use crate::ai::goals::{pick_best, ScoredGoal};
use crate::ai::needs::{heal, take_damage, urgency};
use crate::ai::observability::{AiIntrospection, SimTestConfig, SimTestTier, StabilityMetrics};
use crate::ai::offline_simulation::offline_combat;
use crate::ai::social::teach_skill;
use crate::ai::traits::{monster_trait_weight, npc_trait_weight};
...
impl EngineSystem for AiDecisionWireSystem {
    ...
    fn fixed_tick(&mut self, ctx: &mut FixedTickContext) {
        ...
        if frame % 200 == 0 {
            for &entity in &ctx.ecs.alive {
                if let Some(g) = crate::ai::groups::find_or_form_group(ctx.ecs, entity) {
                    let _ = g.leader;
                    let _ = g.members;
                    let _ = g.formed_tick;
                }
