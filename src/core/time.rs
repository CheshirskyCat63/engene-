...
impl Season {
    pub fn food_regen_mult(&self) -> f32 {
        match self { Self::Spring => 1.5, Self::Summer => 1.2, Self::Autumn => 0.8, Self::Winter => 0.3 }
    }

    pub fn hunger_drain_mult(&self) -> f32 {
        match self { Self::Spring => 1.0, Self::Summer => 1.0, Self::Autumn => 1.1, Self::Winter => 1.4 }
    }

    pub fn breeding_mult(&self) -> f32 {
        match self { Self::Spring => 1.5, Self::Summer => 1.0, Self::Autumn => 0.5, Self::Winter => 0.0 }
    }

    pub fn danger_mult(&self) -> f32 {
        match self { Self::Spring => 1.0, Self::Summer => 1.0, Self::Autumn => 1.1, Self::Winter => 1.5 }
    }
}
