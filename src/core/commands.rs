...
pub enum ComponentOp {
    Add { entity: Entity, type_name: &'static str, data: Box<dyn Any + Send + Sync> },
    Remove { entity: Entity, type_name: &'static str },
}
...
impl CommandBuffer {
    ...
    pub fn spawn_count(&self) -> usize { self.spawns.len() }
    pub fn despawn_count(&self) -> usize { self.despawns.len() }
    pub fn component_op_count(&self) -> usize { self.component_ops.len() }
    pub fn event_count(&self) -> usize { self.deferred_events.len() }
    pub fn is_empty(&self) -> bool {
        self.spawns.is_empty() && self.despawns.is_empty()
            && self.component_ops.is_empty() && self.deferred_events.is_empty()
    }

    pub fn take_spawns(&mut self) -> Vec<SpawnCommand> { std::mem::take(&mut self.spawns) }
    pub fn take_despawns(&mut self) -> Vec<Entity> { std::mem::take(&mut self.despawns) }
    pub fn take_component_ops(&mut self) -> Vec<ComponentOp> { std::mem::take(&mut self.component_ops) }
    pub fn take_events(&mut self) -> Vec<Box<dyn Any + Send + Sync>> { std::mem::take(&mut self.deferred_events) }
