...
pub enum MetricValue {
    ...
    Histogram { sum: f64, count: u64, min: f64, max: f64 },
}
...
impl MetricsRegistry {
    ...
    pub fn register_counter(&self, subsystem: &str, name: &str, description: &str) {
        if let Ok(mut subs) = self.subsystems.write() {
            ...
            sub.counters.entry(name.to_string()).or_insert_with(AtomicCounter::new);
        }
    ...
    pub fn register_gauge(&self, subsystem: &str, name: &str, description: &str) {
        if let Ok(mut subs) = self.subsystems.write() {
            let sub = subs.entry(subsystem.to_string()).or_default();
            sub.gauges.entry(name.to_string()).or_insert_with(AtomicGauge::default);
        }
        if let Ok(mut descs) = self.descriptions.write() {
            descs.insert(format!("{}.{}", subsystem, name), description.to_string());
        }
    ...
    pub fn export_json(&self) -> String {
        ...
        for (i, metric) in metrics.iter().enumerate() {
            let value_json = match &metric.value {
                ...
                MetricValue::Histogram { sum, count, min, max } => {
                    format!(
...
pub fn create_default_metrics_registry() -> MetricsRegistry {
    ...
    // Engine metrics
    registry.register_counter("engine", "frame_time_us", "Total frame time in microseconds");
    registry.register_counter("engine", "tick_time_us", "Fixed tick time in microseconds");
    registry.register_counter("engine", "systems_serialized_count", "Number of systems that had to run serially");
    registry.register_counter("engine", "parallel_groups_count", "Number of parallel groups detected");
    registry.register_gauge("engine", "fps", "Current frames per second");
    
    // AI metrics
    registry.register_counter("ai", "entities_ticked", "Number of AI entities ticked this frame");
    ...
    // Physics metrics
    registry.register_counter("physics", "rapier_step_us", "Time spent in Rapier physics step");
    registry.register_counter("physics", "fire_cells_active", "Active fire simulation cells");
    registry.register_counter("physics", "water_cells_active", "Active water simulation cells");
    registry.register_counter("physics", "cloth_particles", "Active cloth particles");
    registry.register_counter("physics", "impacts_resolved", "Damage impacts resolved this frame");
    ...
    registry.register_gauge("graphics", "degradation_level", "Current quality degradation level");
    ...
    registry.register_counter("world", "entities_streamed_in", "Entities streamed in this frame");
    registry.register_counter("world", "entities_streamed_out", "Entities streamed out this frame");
    ...
    // Streaming metrics
    registry.register_counter("streaming", "read_bytes_frame", "Bytes read from disk this frame");
    registry.register_counter("streaming", "decompress_bytes_frame", "Bytes decompressed this frame");
    registry.register_counter("streaming", "upload_bytes_frame", "Bytes uploaded to GPU this frame");
