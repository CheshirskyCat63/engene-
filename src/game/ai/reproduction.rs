...
pub fn try_reproduce(ecs: &mut Ecs, parent: Entity, mate: Entity) -> Option<Entity> {
    ...
    ecs.inventories
        .insert(child, Inventory { items: Vec::new() });
    ...
    match &kind {
        EntityKind::Npc => {
            ...
            ecs.personal_needs
                .insert(child, PersonalNeeds::default_npc());
        ...
        EntityKind::Monster(species) => {
            ...
            ecs.monster_traits.insert(
                child,
                blend_monster_traits(ecs, parent, mate, *species, &mut rng),
            );
            ecs.personal_needs
                .insert(child, PersonalNeeds::default_monster());
            ecs.ecosystem_needs
                .insert(child, EcosystemNeeds::for_species(*species));
            ecs.life_info
                .insert(child, LifeInfo::new_monster(*species).with_age(0.0));
        }
    }

    // Mark mate cooldown on both parents
    let current_day = ecs.tick as u32 / 120;
    if let Some(li) = ecs.get_life_info_mut(parent) {
        li.last_mate_day = current_day;
    }
    if let Some(li) = ecs.get_life_info_mut(mate) {
        li.last_mate_day = current_day;
    }

    if let Some(mem) = ecs.get_memory_mut(parent) {
        mem.adjust_opinion(child_pid, |op| {
            op.trust = 0.8;
            op.familiarity = 0.5;
        });
    }
...
fn blend_monster_traits(
    ...
    let base = match species {
        MonsterSpecies::Wolf => MonsterTraits {
            aggressiveness: 0.6,
            caution: 0.4,
            territoriality: 0.5,
            bravery: 0.5,
        ...
        MonsterSpecies::Boar => MonsterTraits {
            aggressiveness: 0.3,
            caution: 0.6,
            territoriality: 0.3,
            bravery: 0.3,
        ...
        MonsterSpecies::Bloodsucker => MonsterTraits {
            aggressiveness: 0.9,
            caution: 0.2,
            territoriality: 0.8,
            bravery: 0.7,
