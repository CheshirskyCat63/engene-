...
pub fn desire_npc(entity: Entity, ecs: &Ecs, day_progress: f32) -> Goal {
    let pn = match ecs.get_needs(entity) {
        Some(p) => p,
        None => return Goal::Rest,
    };
    let traits = match ecs.get_npc_traits(entity) {
        Some(t) => t,
        None => return Goal::Rest,
    };
    let sn = match ecs.get_social_needs(entity) {
        Some(s) => s,
        None => return Goal::Rest,
    };
    ...
    let emo = match ecs.get_emotions(entity) {
        Some(e) => e,
        None => return Goal::Rest,
    };
    ...
    if pn.health < th.health_panic {
        return if emo.fear > 0.3 || traits.bravery < 0.4 {
            Goal::Flee
        } else {
            Goal::Rest
        };
    ...
    if pn.hunger > th.hunger_critical {
        ...
        return if conf > 0.4 && traits.aggressiveness > 0.3 {
            Goal::Hunt
        } else {
            Goal::SeekFood
        };
    }
    if pn.thirst > th.thirst_critical {
        return Goal::SeekWater;
    }
    ...
    if is_night && pn.sleep > th.sleep_critical {
        let at_shelter = mem.map_or(false, |m| {
            if let Some(t) = ecs.get_transform(entity) {
                m.spatial
                    .get(&(t.cell_x, t.cell_y))
                    .map_or(false, |k| k.shelter > 0.3)
            } else {
                false
            }
        });
        return if at_shelter {
            Goal::Sleep
        } else {
            Goal::SeekShelter
        };
    ...
    match emo.dominant() {
        DominantEmotion::Anger => {
            if traits.aggressiveness > 0.4 {
                return if econ.desperation > th.desperation_crime && traits.honesty < 0.4 {
                    Goal::StealOrRob
                } else {
                    Goal::Hunt
                };
        ...
        DominantEmotion::Grief | DominantEmotion::Longing => {
            if traits.sociality > 0.4 {
                return Goal::Socialize;
            }
        }
        DominantEmotion::Fear => {
            if traits.bravery < 0.5 {
                return Goal::Flee;
            }
        }
        _ => {}
    ...
    if econ.desperation > 0.6 {
        if traits.honesty < 0.4 && traits.risk_tolerance > 0.5 {
            let steal_score = mem.map_or(0.5, |m| {
                m.lesson_score(LessonAction::Steal, LessonContext::General)
            });
            if steal_score > 0.3 {
                return Goal::StealOrRob;
            }
        }
        return Goal::Work;
    }
    ...
    if sn.money > 0.5 && traits.work_ethic > 0.5 {
        return Goal::Work;
    }
    if sn.money > 0.4 && traits.materialism > 0.5 {
        return Goal::Trade;
    }

    if pn.curiosity > 0.5 && traits.curiosity > 0.4 {
        return Goal::Explore;
    }
    if pn.ambitions > 0.5 {
        return if traits.work_ethic > traits.aggressiveness {
            Goal::Work
        } else {
            Goal::Hunt
        };
    }
    if sn.entertainment > 0.5 {
        return Goal::Socialize;
    }
...
pub fn desire_monster(entity: Entity, ecs: &Ecs, day_progress: f32) -> Goal {
    ...
    let traits = match ecs.get_monster_traits(entity) {
        Some(t) => t,
        None => return Goal::Rest,
    };
    let eco = match ecs.get_ecosystem_needs(entity) {
        Some(e) => e,
        None => return Goal::Rest,
    };
    let emo = match ecs.get_emotions(entity) {
    ...
    let is_nocturnal = matches!(
        ecs.get_kind(entity),
        Some(EntityKind::Monster(MonsterSpecies::Bloodsucker))
    );

    if pn.health < th.health_panic {
        return if traits.bravery > 0.7 {
    ...
    if emo.fear > th.fear_flee && traits.caution > 0.3 {
        return if traits.pack_mentality > 0.5 {
            Goal::FollowPack
        } else {
            Goal::Flee
        };
    ...
    if pn.hunger > th.hunger_critical {
        ...
        return if conf > 0.3 && traits.aggressiveness > 0.2 {
            Goal::Hunt
        } else {
            Goal::SeekFood
        };
    ...
    match emo.dominant() {
        DominantEmotion::Anger => {
            return if eco.territory_control > 0.4 {
                Goal::DefendTerritory
            } else {
                Goal::Hunt
            };
        ...
        DominantEmotion::Grief | DominantEmotion::Longing => {
            if traits.pack_mentality > 0.4 {
                return Goal::FollowPack;
            }
        }
        DominantEmotion::Fear => {
            return Goal::Flee;
    ...
    if eco.territory_control > 0.5 && traits.territoriality > 0.5 {
        return Goal::DefendTerritory;
    }
    if eco.pack_following > 0.6 && traits.pack_mentality > 0.5 {
        return Goal::FollowPack;
    }

    if pn.hunger > th.hunger_proactive && traits.aggressiveness > 0.3 {
        return Goal::Hunt;
    }
    if eco.migration_urge > 0.5 || eco.resource_competition > 0.6 {
        return Goal::Migrate;
    }
    if pn.hunger > 0.3 {
        return Goal::SeekFood;
    }
    if traits.curiosity > 0.3 && pn.curiosity > 0.3 {
        return Goal::Explore;
    }
