...
use crate::game::ai::memory::CellTag;
use crate::game::ai::perception::PerceptionCache;
use crate::game::ai::{body, combat, desire, emotions, needs, perception, witness};
use crate::world::cell::CELL_SIZE;
use crate::world::components::*;
use crate::world::resources::ResourceGrid;

/// Tick a single monster entity.
///
/// Uses query API methods instead of direct storage access.
pub fn tick_monster(
    ecs: &mut Ecs,
    events: &mut EventBus,
    res: &mut ResourceGrid,
    entity: Entity,
    delta: f32,
    day_progress: f32,
) {
    ...
    // Use helper method for nocturnal trait check
    let is_nocturnal = ecs.is_nocturnal(entity);
    ...
    if should_replan {
        ...
        ecs.set_plan(entity, Plan::new(new_goal, target, ecs.tick));
    }

    if let Some(mut plan) = ecs.take_plan(entity) {
        plan.tick(delta);
        // Use helper methods
        let stage = ecs
            .get_life_info(entity)
        ...
        execute(
            ecs, events, res, entity, &plan, delta, &body_st, tod, &pcache,
        );
        if ecs.is_alive(entity) {
            ecs.set_ai_state(entity, AiState::Executing(plan.goal));
            if !plan.is_expired() {
                ecs.set_plan(entity, plan);
            }
        }
    }
}
...
fn should_replan(ecs: &Ecs, entity: Entity, new_goal: Goal) -> bool {
    // Use helper method for current plan lookup
...
fn find_target_for_goal(
    ecs: &Ecs,
    res: &ResourceGrid,
    entity: Entity,
    goal: Goal,
    pcache: &PerceptionCache,
) -> Option<(f32, f32)> {
    // Use helper methods instead of direct storage access
    let t = ecs.get_transform(entity)?;
    let mem = ecs.get_memory(entity);

    match goal {
        Goal::SeekFood => {
            if let Some(pos) = res.find_nearest_carcass(t.x, t.y, 250.0) {
                return Some(pos);
            }
            if let Some(cell) = mem_food_cell(mem) {
                return Some(cell_center(cell.0, cell.1));
            }
            res.best_food_cell(t.cell_x, t.cell_y, 5)
                .map(|(cx, cy)| cell_center(cx, cy))
        }
        Goal::Hunt => pcache
            .prey
            .and_then(|p| ecs.get_transform(p).map(|pt| (pt.x, pt.y))),
        Goal::Flee => {
            let safe = mem.and_then(|m| {
                m.spatial
                    .iter()
                    .filter(|(_, k)| k.danger < 0.1)
            ...
            safe.or_else(|| {
                pcache
                    .predator
                    .and_then(|p| ecs.get_transform(p))
                    .map(|pt| {
                        (
                            t.x + (t.x - pt.x).signum() * 120.0,
                            t.y + (t.y - pt.y).signum() * 120.0,
                        )
                    })
            })
        }
        Goal::FollowPack => {
            ...
            if let Some(a) = trusted {
                ecs.get_transform(a).map(|at| (at.x, at.y))
            } else {
                pcache
                    .nearby_allies
                    .first()
                    .and_then(|&a| ecs.get_transform(a).map(|at| (at.x, at.y)))
            }
        }
        Goal::Migrate => {
            let mut rng = rand::thread_rng();
            Some(cell_center(
                rng.gen_range(0..crate::world::cell::GRID_SIZE),
                rng.gen_range(0..crate::world::cell::GRID_SIZE),
            ))
        }
        Goal::Explore => {
        ...
        Goal::Mate => perception::find_mate(ecs, entity)
...
fn decay_and_sense(
    ...
    // Use helper methods instead of direct storage access
    let stress_tol = ecs
        .get_monster_traits(entity)
        .map_or(0.5, |t| t.stress_tolerance);
    let energy_lvl = ecs
        .get_monster_traits(entity)
        .map_or(0.7, |t| t.energy_level);
    let bravery = ecs.get_monster_traits(entity).map_or(0.5, |t| t.bravery);
    let caution = ecs.get_monster_traits(entity).map_or(0.5, |t| t.caution);
    let pack_ment = ecs
        .get_monster_traits(entity)
        .map_or(0.5, |t| t.pack_mentality);
    let adaptability = ecs
        .get_monster_traits(entity)
        .map_or(0.5, |t| t.adaptability);
    let is_night = body::is_night(day_progress);
    let learning_mult = ecs
        .get_life_info(entity)
        .map_or(1.0, |li| li.life_stage().learning_mult());

    let night_sleep_mult = if is_nocturnal {
        if is_night {
            0.5
        } else {
            2.0
    ...
    if let Some(eco) = ecs.get_ecosystem_needs_mut(entity) {
    ...
    if let Some(emo) = ecs.get_emotions_mut(entity) {
        emo.decay(delta);
        if is_night && !is_nocturnal && bravery < 0.4 {
            emo.fear = (emo.fear + delta * 0.003).min(1.0);
        }
    }
    if let Some(mem) = ecs.get_memory_mut(entity) {
        mem.decay_spatial(delta * 0.0005);
    }

    if let Some(predator) = pcache.predator {
        let traits_snapshot = ecs.get_monster_traits(entity).cloned();
        if let (Some(emo), Some(traits)) = (ecs.get_emotions_mut(entity), traits_snapshot.as_ref())
        {
            emotions::apply_monster_personality(emo, traits, 0.0, delta * 0.15, 0.0, 0.0);
        }
        if let Some(pn) = ecs.get_needs_mut(entity) {
            pn.fear = (pn.fear + delta * 0.12 * caution * (1.0 - bravery)).min(1.0);
        }
        if let Some(eco) = ecs.get_ecosystem_needs_mut(entity) {
            eco.predator_avoidance = (eco.predator_avoidance + delta * 0.05).min(1.0);
        }
    ...
    if let Some(eco) = ecs.get_ecosystem_needs_mut(entity) {
        eco.resource_competition = if density > 2 {
            (eco.resource_competition + delta * 0.008 * (1.0 - adaptability)).min(1.0)
        } else {
            (eco.resource_competition - delta * 0.005 * adaptability).max(0.0)
        };
    }

    if !pcache.nearby_allies.is_empty() {
        if let Some(pn) = ecs.get_needs_mut(entity) {
            pn.fear = (pn.fear - delta * 0.005 * pcache.nearby_allies.len() as f32).max(0.0);
        }
        if let Some(emo) = ecs.get_emotions_mut(entity) {
            emo.longing = (emo.longing - delta * 0.01).max(0.0);
        }
        ...
        if let Some(emo) = ecs.get_emotions_mut(entity) {
            emo.longing = (emo.longing + delta * 0.005 * pack_ment).min(1.0);
        }
    }

    if let Some(loc) = ecs.get_transform(entity).map(|t| (t.cell_x, t.cell_y)) {
        if let Some(mem) = ecs.get_memory_mut(entity) {
            mem.mark_cell(loc.0, loc.1, CellTag::Shelter, 0.02);
        }
    }
}
...
fn execute(
    ...
    match plan.goal {
        Goal::Hunt => {
            ...
            let aggr = ecs
                .get_monster_traits(entity)
                .map_or(0.5, |t| t.aggressiveness);
            let pack_ment = ecs
                .get_monster_traits(entity)
                .map_or(0.5, |t| t.pack_mentality);
            let prey_sel = ecs
                .get_ecosystem_needs(entity)
                .map_or(0.5, |e| e.prey_selection);
            ...
            if let Some(prey) = perception::find_prey_selective(ecs, entity, prey_sel) {
                move_toward(
                    ...
                    entity,
                    prey,
                    speed * (4.0 + aggr * 3.0) * body.combat_power_mult,
                );
                ...
                if !allies.is_empty() {
                    if let Some(target) = perception::find_group_target(ecs, allies, entity) {
                        ...
                        if perception::distance2(ecs, entity, target) < 35.0 * 35.0 {
                            let mut group: Vec<Entity> = allies
                                .iter()
                                .copied()
                                .filter(|&a| perception::distance2(ecs, a, target) < 50.0 * 50.0)
                                .collect();
                            group.push(entity);
                            combat::resolve_group_combat(ecs, events, &group, target);
                        }
                    } else {
                        wander(ecs, entity, speed * 3.0);
                    }
                } else {
                    let trusted = ecs
                        .get_memory(entity)
                    ...
                    if let Some(ally) = trusted {
                        move_toward(ecs, entity, ally, speed * 3.0);
                    } else {
                        wander(ecs, entity, speed * 3.0);
                    }
                }
            ...
            if let Some(eco) = ecs.get_ecosystem_needs_mut(entity) {
                eco.hunting = (eco.hunting - delta * 0.01).max(0.0);
            }
            if let Some(pn) = ecs.get_needs_mut(entity) {
                pn.energy = (pn.energy - delta * 0.015).max(0.0);
            }
        }
        Goal::Flee => {
            if let Some(target) = plan.target_pos {
                move_to_pos(ecs, entity, target, speed * 10.0);
            } else {
                flee_from(ecs, entity, pcache.predator, speed * 10.0);
            }
            if let Some(pn) = ecs.get_needs_mut(entity) {
                pn.fear = (pn.fear - delta * 0.03).max(0.0);
                pn.energy = (pn.energy - delta * 0.015).max(0.0);
            }
            if let Some(emo) = ecs.get_emotions_mut(entity) {
                emo.fear = (emo.fear - delta * 0.005).max(0.0);
            }
        }
        Goal::Rest => {
            let energy_lvl = ecs
            ...
            if let Some(emo) = ecs.get_emotions_mut(entity) {
                emo.anger = (emo.anger - delta * 0.003).max(0.0);
            }
        }
        Goal::Sleep => {
            let energy_lvl = ecs
        ...
        Goal::SeekShelter => {
            if let Some(target) = plan.target_pos {
                move_to_pos(ecs, entity, target, speed * 4.0);
            } else {
                if let Some(&leader) = pcache.nearby_allies.first() {
                    move_toward(ecs, entity, leader, speed * 3.0);
                } else {
                    wander(ecs, entity, speed * 3.0);
                }
            }
            ...
            if let Some(t) = ecs.get_transform(entity).map(|t| (t.cell_x, t.cell_y)) {
                if let Some(mem) = ecs.get_memory_mut(entity) {
                    mem.mark_cell(t.0, t.1, CellTag::Shelter, 0.1);
                }
            }
        }
        Goal::DefendTerritory => {
            ...
            if let Some(eco) = ecs.get_ecosystem_needs_mut(entity) {
                eco.territory_control = (eco.territory_control - delta * 0.015).max(0.0);
            }
        }
        Goal::Migrate => {
            if let Some(target) = plan.target_pos {
                move_to_pos(ecs, entity, target, speed * 6.0);
            } else {
                wander(ecs, entity, speed * 6.0);
            }
            if let Some(eco) = ecs.get_ecosystem_needs_mut(entity) {
                eco.migration_urge = (eco.migration_urge - delta * 0.02).max(0.0);
                eco.resource_competition = (eco.resource_competition - delta * 0.01).max(0.0);
            }
        }
        Goal::FollowPack => {
            if let Some(target) = plan.target_pos {
                move_to_pos(ecs, entity, target, speed * 3.0);
            } else {
                if let Some(&leader) = pcache.nearby_allies.first() {
                    move_toward(ecs, entity, leader, speed * 3.0);
                } else {
                    wander(ecs, entity, speed * 2.0);
                }
            }
            if let Some(eco) = ecs.get_ecosystem_needs_mut(entity) {
                eco.pack_following = (eco.pack_following - delta * 0.01).max(0.0);
            }
            if let Some(emo) = ecs.get_emotions_mut(entity) {
                emo.longing = (emo.longing - delta * 0.01).max(0.0);
            }
        ...
        Goal::Explore => {
            if let Some(target) = plan.target_pos {
                move_to_pos(ecs, entity, target, speed * 5.0);
            } else {
                wander(ecs, entity, speed * 5.0);
            }
            if let Some(pn) = ecs.get_needs_mut(entity) {
                pn.curiosity = (pn.curiosity - delta * 0.008).max(0.0);
            }
            if let Some(t) = ecs.get_transform(entity).map(|t| (t.cell_x, t.cell_y)) {
                let food = res.food_at(t.0, t.1);
                if let Some(mem) = ecs.get_memory_mut(entity) {
                    if food > 0.2 {
                        mem.mark_cell(t.0, t.1, CellTag::Food, food * 0.5);
                    }
                }
            }
        }
        Goal::SeekFood => {
            ...
            let hoarding = ecs.get_monster_traits(entity).map_or(0.3, |t| t.hoarding);
            if let Some(t) = ecs.get_transform(entity) {
                let (tx, ty, cell_x, cell_y) = (t.x, t.y, t.cell_x, t.cell_y);
                let from_carcass = res.consume_carcass_near(tx, ty, delta * 0.06);
                if from_carcass > 0.0 {
                    if let Some(pn) = ecs.get_needs_mut(entity) {
                        needs::satisfy_hunger(pn, from_carcass * (1.0 + hoarding * 0.3));
                    }
                    if let Some(mem) = ecs.get_memory_mut(entity) {
                        mem.mark_cell(cell_x, cell_y, CellTag::Food, 0.4);
                    }
                } else {
                    let taken = res.consume_food(cell_x, cell_y, delta * (0.02 + hoarding * 0.01));
                    if taken > 0.01 {
                        if let Some(pn) = ecs.get_needs_mut(entity) {
                            needs::satisfy_hunger(pn, taken);
                        }
                        if let Some(mem) = ecs.get_memory_mut(entity) {
                            mem.mark_cell(cell_x, cell_y, CellTag::Food, 0.2);
                        }
                    }
                }
            }
        ...
        Goal::Mate => {
            if let Some(mate) = perception::find_mate(ecs, entity) {
                ...
                if perception::distance2(ecs, entity, mate) < 20.0 * 20.0 {
                    crate::game::ai::reproduction::try_reproduce(ecs, entity, mate);
                    if let Some(emo) = ecs.get_emotions_mut(entity) {
                        emo.joy = (emo.joy + 0.15).min(1.0);
                    }
                }
            } else {
                wander(ecs, entity, speed * 2.0);
...
fn mem_food_cell(mem: Option<&crate::core::ai_memory::Memory>) -> Option<(u32, u32)> {
    mem?.spatial
        .iter()
...
fn entities_in_radius(ecs: &Ecs, origin: Entity, radius: f32) -> Vec<Entity> {
    ...
    let kind = ecs.get_kind(origin);
    ecs.spatial
        .candidates_in_radius(ot.x, ot.y, radius)
        .into_iter()
        .filter(|&e| {
            if e == origin {
                return false;
...
fn move_toward(ecs: &mut Ecs, mover: Entity, target: Entity, speed: f32) {
    // Use helper method
    let (tx, ty) = match ecs.get_transform(target) {
        Some(t) => (t.x, t.y),
        None => return,
    };
...
fn flee_from(ecs: &mut Ecs, entity: Entity, threat: Option<Entity>, speed: f32) {
    ...
    let (dx, dy) = if let Some(t) = threat {
        let (tx, ty) = match ecs.get_transform(t) {
            Some(tr) => (tr.x, tr.y),
            None => (rng.gen_range(-1.0..1.0), rng.gen_range(-1.0..1.0)),
        };
        let (mx, my) = match ecs.get_transform(entity) {
            Some(tr) => (tr.x, tr.y),
            None => return,
        };
        (mx - tx, my - ty)
