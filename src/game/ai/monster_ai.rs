...
pub fn tick_monster(ecs: &mut Ecs, events: &mut EventBus, res: &mut ResourceGrid, entity: Entity, delta: f32, day_progress: f32) {
    ...
    if let Some(mut plan) = ecs.take_plan(entity) {
        ...
        let body_st = ecs.get_needs(entity)
        ...
        if ecs.is_alive(entity) {
            ...
            if !plan.is_expired() { ecs.set_plan(entity, plan); }
        }
    }
}
...
fn find_target_for_goal(ecs: &Ecs, res: &ResourceGrid, entity: Entity, goal: Goal, pcache: &PerceptionCache) -> Option<(f32, f32)> {
    ...
    match goal {
        ...
        Goal::Hunt => {
            pcache.prey.and_then(|p| ecs.get_transform(p).map(|pt| (pt.x, pt.y)))
        }
        Goal::Flee => {
        ...
        Goal::FollowPack => {
            ...
            if let Some(a) = trusted {
                ...
                pcache.nearby_allies.first().and_then(|&a| ecs.get_transform(a).map(|at| (at.x, at.y)))
            }
        }
        Goal::Migrate => {
...
fn decay_and_sense(ecs: &mut Ecs, entity: Entity, delta: f32, day_progress: f32, is_nocturnal: bool, pcache: &PerceptionCache) {
    // Use helper methods instead of direct storage access
    let stress_tol = ecs.get_monster_traits(entity).map_or(0.5, |t| t.stress_tolerance);
    let energy_lvl = ecs.get_monster_traits(entity).map_or(0.7, |t| t.energy_level);
    ...
    let pack_ment = ecs.get_monster_traits(entity).map_or(0.5, |t| t.pack_mentality);
    let adaptability = ecs.get_monster_traits(entity).map_or(0.5, |t| t.adaptability);
    ...
    if let Some(predator) = pcache.predator {
        ...
        if let Some(pn) = ecs.get_needs_mut(entity) { pn.fear = (pn.fear + delta * 0.12 * caution * (1.0 - bravery)).min(1.0); }
        if let Some(eco) = ecs.get_ecosystem_needs_mut(entity) { eco.predator_avoidance = (eco.predator_avoidance + delta * 0.05).min(1.0); }
        if let Some(loc) = ecs.get_transform(predator).map(|t| (t.cell_x, t.cell_y)) {
            if let Some(mem) = ecs.get_memory_mut(entity) { mem.mark_cell(loc.0, loc.1, CellTag::Danger, 0.3); }
        }
    }
    ...
    if !pcache.nearby_allies.is_empty() {
        if let Some(pn) = ecs.get_needs_mut(entity) { pn.fear = (pn.fear - delta * 0.005 * pcache.nearby_allies.len() as f32).max(0.0); }
        ...
        if let Some(emo) = ecs.get_emotions_mut(entity) { emo.longing = (emo.longing + delta * 0.005 * pack_ment).min(1.0); }
    }
...
fn execute(ecs: &mut Ecs, events: &mut EventBus, res: &mut ResourceGrid, entity: Entity, plan: &Plan, delta: f32, body: &BodyState, tod: f32, pcache: &PerceptionCache) {
    ...
    match plan.goal {
        Goal::Hunt => {
            // Use helper methods
            let aggr = ecs.get_monster_traits(entity).map_or(0.5, |t| t.aggressiveness);
            let pack_ment = ecs.get_monster_traits(entity).map_or(0.5, |t| t.pack_mentality);
            let prey_sel = ecs.get_ecosystem_needs(entity).map_or(0.5, |e| e.prey_selection);
            ...
            if let Some(eco) = ecs.get_ecosystem_needs_mut(entity) { eco.hunting = (eco.hunting - delta * 0.01).max(0.0); }
            if let Some(pn) = ecs.get_needs_mut(entity) { pn.energy = (pn.energy - delta * 0.015).max(0.0); }
        }
        Goal::Flee => {
            ...
            if let Some(pn) = ecs.get_needs_mut(entity) { pn.fear = (pn.fear - delta * 0.03).max(0.0); pn.energy = (pn.energy - delta * 0.015).max(0.0); }
        ...
        Goal::Rest => {
            ...
            if let Some(emo) = ecs.get_emotions_mut(entity) { emo.anger = (emo.anger - delta * 0.003).max(0.0); }
        }
        Goal::Sleep => {
            let energy_lvl = ecs.get_monster_traits(entity).map_or(0.7, |t| t.energy_level);
            if let Some(pn) = ecs.get_needs_mut(entity) {
        ...
        Goal::DefendTerritory => {
            ...
            if let Some(eco) = ecs.get_ecosystem_needs_mut(entity) { eco.territory_control = (eco.territory_control - delta * 0.015).max(0.0); }
        }
        Goal::Migrate => {
            if let Some(target) = plan.target_pos { move_to_pos(ecs, entity, target, speed * 6.0); }
            else { wander(ecs, entity, speed * 6.0); }
            if let Some(eco) = ecs.get_ecosystem_needs_mut(entity) { eco.migration_urge = (eco.migration_urge - delta * 0.02).max(0.0); eco.resource_competition = (eco.resource_competition - delta * 0.01).max(0.0); }
        }
        Goal::FollowPack => {
            ...
            if let Some(eco) = ecs.get_ecosystem_needs_mut(entity) { eco.pack_following = (eco.pack_following - delta * 0.01).max(0.0); }
            if let Some(emo) = ecs.get_emotions_mut(entity) { emo.longing = (emo.longing - delta * 0.01).max(0.0); }
        }
        Goal::Explore => {
            if let Some(target) = plan.target_pos { move_to_pos(ecs, entity, target, speed * 5.0); }
            else { wander(ecs, entity, speed * 5.0); }
            if let Some(pn) = ecs.get_needs_mut(entity) { pn.curiosity = (pn.curiosity - delta * 0.008).max(0.0); }
            if let Some(t) = ecs.get_transform(entity).map(|t| (t.cell_x, t.cell_y)) {
                let food = res.food_at(t.0, t.1);
                if let Some(mem) = ecs.get_memory_mut(entity) { if food > 0.2 { mem.mark_cell(t.0, t.1, CellTag::Food, food * 0.5); } }
            }
        }
        Goal::SeekFood => {
            if let Some(target) = plan.target_pos { move_to_pos(ecs, entity, target, speed * 4.0); }
            ...
            if let Some(t) = ecs.get_transform(entity) {
                ...
                if from_carcass > 0.0 {
                    if let Some(pn) = ecs.get_needs_mut(entity) { needs::satisfy_hunger(pn, from_carcass * (1.0 + hoarding * 0.3)); }
                    if let Some(mem) = ecs.get_memory_mut(entity) { mem.mark_cell(cell_x, cell_y, CellTag::Food, 0.4); }
                } else {
                    ...
                    if taken > 0.01 {
                        ...
                        if let Some(mem) = ecs.get_memory_mut(entity) { mem.mark_cell(cell_x, cell_y, CellTag::Food, 0.2); }
                    }
                }
            }
        }
        Goal::Mate => {
            if let Some(mate) = perception::find_mate(ecs, entity) {
                ...
                if perception::distance2(ecs, entity, mate) < 20.0 * 20.0 {
                    ...
                    if let Some(emo) = ecs.get_emotions_mut(entity) { emo.joy = (emo.joy + 0.15).min(1.0); }
                }
            } else { wander(ecs, entity, speed * 2.0); }
        }
        _ => {}
...
fn entities_in_radius(ecs: &Ecs, origin: Entity, radius: f32) -> Vec<Entity> {
    // Use helper methods
    let ot = match ecs.get_transform(origin) { Some(t) => t, None => return Vec::new() };
    ...
    ecs.spatial.candidates_in_radius(ot.x, ot.y, radius)
        ...
        .filter(|&e| {
            ...
            ecs.get_transform(e).map_or(false, |t| (t.x - ot.x).powi(2) + (t.y - ot.y).powi(2) < r2)
        })
        .collect()
}
