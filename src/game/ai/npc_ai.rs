        ...
        Goal::SeekFood => {
            ...
            if let Some(t) = ecs.get_transform(entity) {
                ...
                if from_carcass > 0.0 {
                    ...
                    if let Some(mem) = ecs.get_memory_mut(entity) { mem.mark_cell(cell_x, cell_y, CellTag::Food, 0.3); }
                } else {
            ...
            for &w in &pcache.nearby_npcs {
                ...
                if let Some(entity_pid) = ecs.identity.persistent_id_of(entity) {
                    if let Some(mem) = ecs.get_memory_mut(w) {
                        mem.record_event(crate::game::ai::memory::EventMemory { tick, kind: crate::game::ai::memory::EventKind::SawTheft, location: loc, other: Some(entity_pid), emotional_impact: 0.2 });
...
fn find_unexplored_cell(mem: Option<&crate::core::ai_memory::Memory>, _cx: u32, _cy: u32) -> (u32, u32) {
    let mut rng = rand::thread_rng();
