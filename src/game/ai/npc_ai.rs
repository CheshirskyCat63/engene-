...
use crate::game::ai::{body, combat, desire, emotions, needs, perception, social, witness};
...
fn find_target_for_goal(
    ...
    match goal {
        Goal::SeekFood => {
            if let Some(pos) = res.find_nearest_carcass(t.x, t.y, 200.0) {
                return Some(pos);
            }
            if let Some(cell) = mem_food_cell(mem) {
                return Some(cell_center(cell.0, cell.1));
            }
            res.best_food_cell(t.cell_x, t.cell_y, 4)
                .map(|(cx, cy)| cell_center(cx, cy))
        }
        Goal::SeekWater => {
            if let Some(cell) = mem_water_cell(mem) {
                return Some(cell_center(cell.0, cell.1));
            }
            res.best_water_cell(t.cell_x, t.cell_y, 4)
                .map(|(cx, cy)| cell_center(cx, cy))
        }
        Goal::Hunt => pcache
            .prey
            .and_then(|prey| ecs.get_transform(prey).map(|pt| (pt.x, pt.y))),
        Goal::Flee => {
            ...
            safe.or_else(|| {
                pcache
                    ...
                    .map(|pt| {
                        (
                            t.x + (t.x - pt.x).signum() * 100.0,
                            t.y + (t.y - pt.y).signum() * 100.0,
                        )
                    })
            })
        }
        Goal::Socialize => pcache
            .nearby_npcs
            .first()
...
fn decay_and_sense(
    ...
    let stress_res = ecs
        ...
        .map_or(0.5, |t| t.stress_resistance);
    ...
    if let Some(pn) = ecs.get_needs_mut(entity) {
        ...
        if pn.hunger > 0.7 || pn.thirst > 0.7 {
            pn.discomfort = (pn.discomfort + delta * 0.005).min(1.0);
        }
    }
    ...
    if let Some(emo) = ecs.get_emotions_mut(entity) {
        emo.decay(delta);
        if is_alone {
            emo.longing = (emo.longing + delta * 0.003 * sociality).min(1.0);
        }
        if is_night && bravery < 0.5 {
            emo.fear = (emo.fear + delta * 0.002 * night_fear_mult).min(1.0);
        }
    }
    ...
    if let Some(predator) = pcache.predator {
        let traits_snapshot = ecs.get_npc_traits(entity).cloned();
        ...
        if let Some(loc) = ecs.get_transform(predator).map(|t| (t.cell_x, t.cell_y)) {
            if let Some(mem) = ecs.get_memory_mut(entity) {
                mem.mark_cell(loc.0, loc.1, CellTag::Danger, 0.2);
            }
        }
    }
    ...
    if !pcache.nearby_npcs.is_empty() {
        if let Some(sn) = ecs.get_social_needs_mut(entity) {
            sn.loneliness =
                (sn.loneliness - delta * 0.01 * pcache.nearby_npcs.len() as f32).max(0.0);
        }
    ...
    match plan.goal {
        Goal::SeekFood => {
            ...
            if let Some(t) = ecs.get_transform(entity) {
                ...
                let from_carcass = res.consume_carcass_near(tx, ty, delta * 0.05);
                if from_carcass > 0.0 {
                    if let Some(pn) = ecs.get_needs_mut(entity) {
                        needs::satisfy_hunger(pn, from_carcass);
                    }
                    if let Some(mem) = ecs.get_memory_mut(entity) {
                        mem.mark_cell(cell_x, cell_y, CellTag::Food, 0.3);
                    }
                } else {
                    let taken = res.consume_food(cell_x, cell_y, delta * 0.03);
                    if taken > 0.01 {
                        if let Some(pn) = ecs.get_needs_mut(entity) {
            ...
            if let Some(econ) = ecs.get_npc_economy_mut(entity) {
                econ.money = (econ.money - delta * 0.1).max(0.0);
            }
        }
        Goal::SeekWater => {
            if let Some(target) = plan.target_pos {
        ...
        Goal::Work => {
            let (income, work_ethic) = {
                ...
                (
                    delta * rate * we * body.work_efficiency_mult * rng.gen_range(0.8..1.2),
                    we,
                )
            };
            if let Some(econ) = ecs.get_npc_economy_mut(entity) {
            ...
            if let Some(sn) = ecs.get_social_needs_mut(entity) {
                sn.money = (sn.money - delta * 0.005).max(0.0);
                sn.reputation = (sn.reputation + delta * 0.002).min(1.0);
            }
        }
        Goal::Hunt => {
            ...
            if let Some(prey) = pcache.prey.filter(|&p| ecs.is_alive(p)) {
                move_toward(
                    ...
                    entity,
                    prey,
                    speed * (3.0 + aggr * 2.0) * body.combat_power_mult,
                );
                ...
                if !allies.is_empty() {
                    ...
                    if let Some(ally) = trusted {
                        move_toward(ecs, entity, ally, speed * 2.0);
                    } else {
                        wander(ecs, entity, speed * 3.0);
                    }
                }
            ...
            if let Some(pn) = ecs.get_needs_mut(entity) {
                pn.energy = (pn.energy - delta * 0.01).max(0.0);
            }
        }
        Goal::Trade => {
        ...
        Goal::Explore => {
            if let Some(target) = plan.target_pos {
                ...
                wander(ecs, entity, speed * 4.0);
            }
            if let Some(pn) = ecs.get_needs_mut(entity) {
                pn.curiosity = (pn.curiosity - delta * 0.012).max(0.0);
            }
            if let Some(t) = ecs.get_transform(entity).map(|t| (t.cell_x, t.cell_y)) {
        ...
        Goal::Socialize => {
            ...
            if let Some(emo) = ecs.get_emotions_mut(entity) {
                emo.joy = (emo.joy + delta * 0.005).min(1.0);
                emo.longing = (emo.longing - delta * 0.01).max(0.0);
            }
            for &n in &pcache.nearby_npcs {
                if let Some(pid) = ecs.identity.persistent_id_of(n) {
                    if let Some(mem) = ecs.get_memory_mut(entity) {
                        mem.adjust_opinion(pid, |op| {
                            op.trust = (op.trust + delta * 0.003).min(1.0);
                            op.familiarity = (op.familiarity + delta * 0.005).min(1.0);
                        });
                    }
                }
            }
        ...
        Goal::Flee => {
            if let Some(target) = plan.target_pos {
                move_to_pos(ecs, entity, target, speed * 8.0);
            } else {
                flee_from(ecs, entity, pcache.predator, speed * 8.0);
            }
            if let Some(pn) = ecs.get_needs_mut(entity) {
                pn.fear = (pn.fear - delta * 0.02).max(0.0);
                pn.energy = (pn.energy - delta * 0.02).max(0.0);
            }
            if let Some(emo) = ecs.get_emotions_mut(entity) {
                emo.fear = (emo.fear - delta * 0.005).max(0.0);
            }
        ...
        Goal::StealOrRob => {
            ...
            if let Some(sn) = ecs.get_social_needs_mut(entity) {
                sn.reputation = (sn.reputation - delta * 0.02).max(0.0);
                sn.fear_of_punishment = (sn.fear_of_punishment + delta * 0.03).min(1.0);
            }
            if let Some(emo) = ecs.get_emotions_mut(entity) {
                emo.disgust = (emo.disgust + delta * 0.01 * honesty).min(1.0);
            }
            for &w in &pcache.nearby_npcs {
                ...
                if let Some(entity_pid) = ecs.identity.persistent_id_of(entity) {
                    if let Some(mem) = ecs.get_memory_mut(w) {
                        mem.record_event(crate::game::ai::memory::EventMemory {
                            tick,
                            kind: crate::game::ai::memory::EventKind::SawTheft,
                            location: loc,
                            other: Some(entity_pid),
                            emotional_impact: 0.2,
                        });
                        mem.adjust_opinion(entity_pid, |op| {
                            op.trust = (op.trust - 0.15).max(-1.0);
                            op.hostility = (op.hostility + 0.1).min(1.0);
                        });
                    }
                }
                if let Some(emo) = ecs.get_emotions_mut(w) {
                    emo.disgust = (emo.disgust + 0.15).min(1.0);
                }
            }
        }
        Goal::Mate => {
...
fn mem_water_cell(mem: Option<&crate::core::ai_memory::Memory>) -> Option<(u32, u32)> {
    mem?.spatial
        .iter()
...
fn find_unexplored_cell(
    mem: Option<&crate::core::ai_memory::Memory>,
    _cx: u32,
    _cy: u32,
) -> (u32, u32) {
    let mut rng = rand::thread_rng();
