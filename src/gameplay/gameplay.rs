...
use crate::economy::trading::TradeResult;
...
mod tests {
    ...
    fn test_npc_interaction() {
        ...
        let npc = ecs.spawn();
        ecs.kinds.insert(npc, EntityKind::Npc);
        ecs.names.insert(npc, Name("Trader".into()));
        ecs.transforms.insert(npc, Transform { x: 5.0, y: 5.0, cell_x: 0, cell_y: 0 });
        ecs.npc_economies.insert(npc, NpcEconomy { 
            money: 100.0, 
