...
impl EngineConsole {
    ...
    fn register_builtin_commands(&mut self) {
        let engine_cmds: &[(&str, &str, &str)] = &[
            ...
            (
                "runtime_truth",
                "Generate runtime truth JSON",
                "runtime_truth",
            ),
            (
                "world_budget",
                "Display world budget status",
                "world_budget",
            ),
            (
                "spawn",
                "Spawn entity at position",
                "spawn <npc|wolf|boar|bloodsucker> <x> <z>",
            ),
            (
                "teleport",
                "Teleport entity",
                "teleport <entity_id> <x> <z>",
            ),
            ("kill", "Despawn entity", "kill <entity_id>"),
            ("save_chunk", "Save chunk to disk", "save_chunk <cx> <cz>"),
            (
                "reload_chunk",
                "Reload chunk from disk",
                "reload_chunk <cx> <cz>",
            ),
            (
                "time_scale",
        ...
        self.handlers.insert(
            "help".to_string(),
            Box::new(|_| "Use 'help' to list commands".to_string()),
        );
        self.handlers
            .insert("clear".to_string(), Box::new(|_| String::new()));
        for &name in &[
            "scan_entity_refs",
            "runtime_truth",
            "world_budget",
            ...
            self.handlers.insert(
                name.to_string(),
                Box::new(|_| format!("{}: requires engine context", "command")),
            );
        }
    }
        ...
        let output = match parts[0] {
            "scan_entity_refs" => {
                ...
                format!(
                    ...
                    with_pid,
                    if total > 0 {
                        with_pid as f32 / total as f32 * 100.0
                    } else {
                        0.0
                    },
                    without_pid,
            ...
            "doctor" => {
                ...
                format!(
                    ...
                    if matches!(mode, crate::tools::doctor::DoctorMode::Strict) {
                        "strict"
                    } else {
                        "advisory"
                    },
            ...
            "spawn" | "teleport" | "kill" | "save_chunk" | "reload_chunk" | "time_scale"
            | "save_snapshot" => {
    ...
    pub fn execute_with_engine_mut(
        &mut self,
        input: &str,
        engine: &mut crate::core::engine::Engine,
    ) -> String {
        let parts: Vec<&str> = input.trim().split_whitespace().collect();
        if parts.is_empty() {
            return String::new();
                ...
                if parts.len() < 4 {
                    ...
                    engine.ecs.set_transform(
                        ...
                        crate::world::components::Transform {
                            x,
                            y: z,
                            cell_x: (x / 256.0) as u32,
                            cell_y: (z / 256.0) as u32,
                        },
                    );
                    let ek = match kind {
                        "wolf" => crate::world::components::EntityKind::Monster(
                            crate::world::components::MonsterSpecies::Wolf,
                        ),
                        "boar" => crate::world::components::EntityKind::Monster(
                            crate::world::components::MonsterSpecies::Boar,
                        ),
                        "bloodsucker" => crate::world::components::EntityKind::Monster(
                    ...
                    engine.ecs.set_name(
                        entity,
                        crate::world::components::Name(format!("spawned_{}", entity)),
                    );
