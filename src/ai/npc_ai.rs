...
pub fn tick_npc(ecs: &mut Ecs, events: &mut EventBus, res: &mut ResourceGrid, entity: Entity, delta: f32, day_progress: f32) {
    ...
    if let Some(mut plan) = ecs.plans.remove(&entity) {
        ...
        let body = ecs.personal_needs.get(&entity)
...
fn find_target_for_goal(ecs: &Ecs, res: &ResourceGrid, entity: Entity, goal: Goal, pcache: &PerceptionCache) -> Option<(f32, f32)> {
    ...
    match goal {
        ...
        Goal::Hunt => {
            pcache.prey.and_then(|prey| ecs.transforms.get(&prey).map(|pt| (pt.x, pt.y)))
        }
        Goal::Flee => {
        ...
        Goal::Socialize => {
            pcache.nearby_npcs.first()
                .and_then(|&n| ecs.transforms.get(&n).map(|nt| (nt.x, nt.y)))
        }
        Goal::Explore => {
...
fn decay_and_sense(ecs: &mut Ecs, entity: Entity, delta: f32, day_progress: f32, pcache: &PerceptionCache) {
    let stress_res = ecs.npc_traits.get(&entity).map_or(0.5, |t| t.stress_resistance);
    let sociality = ecs.npc_traits.get(&entity).map_or(0.5, |t| t.sociality);
    let bravery = ecs.npc_traits.get(&entity).map_or(0.5, |t| t.bravery);
    ...
    if let Some(sn) = ecs.social_needs.get_mut(&entity) {
    ...
    if let Some(predator) = pcache.predator {
        if let (Some(emo), Some(traits)) = (ecs.emotions.get_mut(&entity), ecs.npc_traits.get(&entity)) {
        ...
        if let Some(loc) = ecs.transforms.get(&predator).map(|t| (t.cell_x, t.cell_y)) {
            if let Some(mem) = ecs.memories.get_mut(&entity) { mem.mark_cell(loc.0, loc.1, CellTag::Danger, 0.2); }
        }
    }
    ...
    match plan.goal {
        Goal::SeekFood => {
            ...
            if let Some(t) = ecs.transforms.get(&entity) {
                ...
                if from_carcass > 0.0 {
                    if let Some(pn) = ecs.personal_needs.get_mut(&entity) { needs::satisfy_hunger(pn, from_carcass); }
                    if let Some(mem) = ecs.memories.get_mut(&entity) { mem.mark_cell(t.cell_x, t.cell_y, CellTag::Food, 0.3); }
                } else {
            ...
            if let Some(econ) = ecs.npc_economies.get_mut(&entity) { econ.money = (econ.money - delta * 0.1).max(0.0); }
        }
        Goal::SeekWater => {
            if let Some(target) = plan.target_pos { move_to_pos(ecs, entity, target, speed * 3.0); }
            else { wander(ecs, entity, speed * 2.0); }
            if let Some(t) = ecs.transforms.get(&entity) {
                ...
                if taken > 0.01 {
                    if let Some(pn) = ecs.personal_needs.get_mut(&entity) { needs::satisfy_thirst(pn, taken); }
                }
            }
        }
        Goal::Rest => {
        ...
        Goal::Work => {
            let (income, work_ethic) = {
                let econ = match ecs.npc_economies.get(&entity) { Some(e) => e, None => return };
                let we = ecs.npc_traits.get(&entity).map_or(0.5, |t| t.work_ethic);
            ...
            if let Some(econ) = ecs.npc_economies.get_mut(&entity) { econ.money += income; }
            if let Some(pn) = ecs.personal_needs.get_mut(&entity) {
                pn.energy = (pn.energy - delta * 0.015 * (2.0 - work_ethic)).max(0.0);
            }
            if let Some(sn) = ecs.social_needs.get_mut(&entity) { sn.money = (sn.money - delta * 0.005).max(0.0); sn.reputation = (sn.reputation + delta * 0.002).min(1.0); }
        }
        Goal::Hunt => {
            let autonomy = ecs.npc_traits.get(&entity).map_or(0.5, |t| t.autonomy);
            let aggr = ecs.npc_traits.get(&entity).map_or(0.5, |t| t.aggressiveness);
            ...
            if let Some(pn) = ecs.personal_needs.get_mut(&entity) { pn.energy = (pn.energy - delta * 0.01).max(0.0); }
        }
        Goal::Trade => {
            if let Some(&partner) = pcache.nearby_npcs.first() {
                ...
                if perception::distance2(ecs, entity, partner) < 25.0 * 25.0 {
                    if social::try_trade(ecs, entity, partner) {
                        if let Some(emo) = ecs.emotions.get_mut(&entity) { emo.joy = (emo.joy + 0.1).min(1.0); }
                    }
                }
            } else { wander(ecs, entity, speed * 2.0); }
        }
        Goal::Explore => {
            ...
            if let Some(pn) = ecs.personal_needs.get_mut(&entity) { pn.curiosity = (pn.curiosity - delta * 0.012).max(0.0); }
            if let Some(t) = ecs.transforms.get(&entity).map(|t| (t.cell_x, t.cell_y)) {
        ...
        Goal::Socialize => {
            ...
            if let Some(emo) = ecs.emotions.get_mut(&entity) { emo.joy = (emo.joy + delta * 0.005).min(1.0); emo.longing = (emo.longing - delta * 0.01).max(0.0); }
        ...
        Goal::Flee => {
            ...
            if let Some(pn) = ecs.personal_needs.get_mut(&entity) { pn.fear = (pn.fear - delta * 0.02).max(0.0); pn.energy = (pn.energy - delta * 0.02).max(0.0); }
            if let Some(emo) = ecs.emotions.get_mut(&entity) { emo.fear = (emo.fear - delta * 0.005).max(0.0); }
        }
        Goal::StealOrRob => {
            let honesty = ecs.npc_traits.get(&entity).map_or(0.5, |t| t.honesty);
            let risk_tol = ecs.npc_traits.get(&entity).map_or(0.5, |t| t.risk_tolerance);
            let stolen = delta * rng.gen_range(1.0..3.0) * (1.0 + risk_tol * 0.5);
            if let Some(econ) = ecs.npc_economies.get_mut(&entity) { econ.money += stolen; }
            if let Some(sn) = ecs.social_needs.get_mut(&entity) { sn.reputation = (sn.reputation - delta * 0.02).max(0.0); sn.fear_of_punishment = (sn.fear_of_punishment + delta * 0.03).min(1.0); }
            if let Some(emo) = ecs.emotions.get_mut(&entity) { emo.disgust = (emo.disgust + delta * 0.01 * honesty).min(1.0); }
            for &w in &pcache.nearby_npcs {
                let tick = ecs.tick;
                let loc = ecs.transforms.get(&entity).map(|t| (t.cell_x, t.cell_y)).unwrap_or((0,0));
                ...
                if let Some(emo) = ecs.emotions.get_mut(&w) { emo.disgust = (emo.disgust + 0.15).min(1.0); }
            }
        }
        Goal::Mate => {
            if let Some(mate) = perception::find_mate(ecs, entity) {
                ...
                if perception::distance2(ecs, entity, mate) < 20.0 * 20.0 {
                    ...
                    if let Some(emo) = ecs.emotions.get_mut(&entity) { emo.joy = (emo.joy + 0.2).min(1.0); }
                }
            } else { wander(ecs, entity, speed * 2.0); }
        }
        _ => {}
