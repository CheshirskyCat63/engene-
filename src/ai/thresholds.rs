...
impl Thresholds {
    pub fn from_npc(t: &NpcTraits, mem: Option<&Memory>) -> Self {
        let near_death_exp = mem.map_or(0.0, |m| {
            m.events.iter().filter(|e| e.kind == crate::ai::memory::EventKind::WasAttacked).count() as f32 * 0.02
        });
    ...
    pub fn from_monster(t: &MonsterTraits, mem: Option<&Memory>) -> Self {
        let near_death_exp = mem.map_or(0.0, |m| {
            m.events.iter().filter(|e| e.kind == crate::ai::memory::EventKind::WasAttacked).count() as f32 * 0.03
        });
