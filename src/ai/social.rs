...
pub fn try_trade(ecs: &mut Ecs, buyer: Entity, seller: Entity) -> bool {
    ...
    if let Some(pn) = ecs.get_needs_mut(buyer) { pn.hunger = (pn.hunger - 0.15).max(0.0); }
    if let Some(pn) = ecs.get_needs_mut(seller) { pn.hunger = (pn.hunger + 0.05).min(1.0); }

    let tick = ecs.tick;
    let loc = ecs.get_transform(buyer).map(|t| (t.cell_x, t.cell_y)).unwrap_or((0, 0));
...
pub fn share_knowledge(ecs: &mut Ecs, speaker: Entity, listener: Entity) {
    ...
    let cells_to_share: Vec<((u32, u32), f32, f32)> = {
        let mem = match ecs.get_memory(speaker) { Some(m) => m, None => return };
        mem.spatial.iter()
    ...
    let tick = ecs.tick;
    let loc = ecs.get_transform(speaker).map(|t| (t.cell_x, t.cell_y)).unwrap_or((0, 0));
...
pub fn teach_skill(ecs: &mut Ecs, teacher: Entity, student: Entity) {
    ...
    let lessons_to_share: Vec<crate::ai::memory::Lesson> = {
        let mem = match ecs.get_memory(teacher) { Some(m) => m, None => return };
        mem.lessons.iter()
