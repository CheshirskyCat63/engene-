...
pub fn resolve_group_combat(ecs: &mut Ecs, events: &mut EventBus, attackers: &[Entity], defender: Entity) {
    ...
    let atk_power: f32 = attackers.iter().map(|&a| {
        let bp = ecs.kinds.get(&a).map_or(1.0, base_power);
        let hp = ecs.personal_needs.get(&a).map_or(1.0, |p| p.health);
        let energy = ecs.personal_needs.get(&a).map_or(0.5, |p| p.energy);
        bp * hp * (0.7 + energy * 0.3)
    }).sum();

    let def_kind = ecs.kinds.get(&defender).cloned();
    let def_bp = def_kind.as_ref().map_or(1.0, base_power);
    let def_hp = ecs.personal_needs.get(&defender).map_or(1.0, |p| p.health);
    let def_energy = ecs.personal_needs.get(&defender).map_or(0.5, |p| p.energy);
    ...
    let predator_bonus = match (attackers.first().and_then(|&a| ecs.kinds.get(&a)), &def_kind) {
    ...
    if let Some(pn) = ecs.personal_needs.get_mut(&defender) {
    ...
    for &a in attackers {
        if let Some(pn) = ecs.personal_needs.get_mut(&a) {
    ...
    let target_dead = ecs.personal_needs.get(&defender).map_or(false, |p| p.health <= 0.0);
    ...
    if target_dead {
        ...
        for &a in attackers {
            ...
            if let Some(econ) = ecs.npc_economies.get_mut(&a) { econ.money += share_loot; }
            if let Some(sn) = ecs.social_needs.get_mut(&a) {
                sn.reputation = (sn.reputation + 0.1).min(1.0);
            }
            if let Some(eco) = ecs.ecosystem_needs.get_mut(&a) {
                eco.food_chain_position = (eco.food_chain_position + 0.05).min(1.0);
                eco.hunting = (eco.hunting - 0.3).max(0.0);
            }
        }

        let def_name = ecs.names.get(&defender).map(|n| n.0.clone()).unwrap_or_default();
        if attackers.len() > 1 {
            let names: Vec<String> = attackers.iter()
                .filter_map(|&a| ecs.names.get(&a).map(|n| n.0.clone())).collect();
            println!("    [GANG KILL] {} ({}) killed {}", names.join("+"), attackers.len(), def_name);
            witness::on_group_kill(ecs, attackers, defender, tick);
        } else {
            let atk_name = attackers.first().and_then(|&a| ecs.names.get(&a)).map(|n| n.0.clone()).unwrap_or_default();
    ...
    for &a in attackers {
        let attacker_dead = ecs.personal_needs.get(&a).map_or(false, |p| p.health <= 0.0);
        if attacker_dead {
            let atk_kind = ecs.kinds.get(&a).cloned();
            let food = atk_kind.as_ref().map_or(0.3, food_value);
            if let Some(pn) = ecs.personal_needs.get_mut(&defender) { pn.hunger = (pn.hunger - food).max(0.0); }
            let atk_name = ecs.names.get(&a).map(|n| n.0.clone()).unwrap_or_default();
            let def_name = ecs.names.get(&defender).map(|n| n.0.clone()).unwrap_or_default();
    ...
    if let Some(loc) = ecs.transforms.get(&defender).map(|t| (t.cell_x, t.cell_y)) {
