...
pub fn tick_monster(ecs: &mut Ecs, events: &mut EventBus, res: &mut ResourceGrid, entity: Entity, delta: f32, day_progress: f32) {
    ...
    // Use helper method instead of direct ecs.kinds.get()
...
fn should_replan(ecs: &Ecs, entity: Entity, new_goal: Goal) -> bool {
    // Use helper method instead of ecs.plans.get()
...
fn decay_and_sense(ecs: &mut Ecs, entity: Entity, delta: f32, day_progress: f32, is_nocturnal: bool, pcache: &PerceptionCache) {
    ...
    if let Some(predator) = pcache.predator {
        if let (Some(emo), Some(traits)) = (ecs.get_emotions_mut(entity), ecs.get_monster_traits(entity)) {
        ...
        Goal::SeekFood => {
            ...
            if let Some(t) = ecs.get_transform(entity) {
                ...
                if from_carcass > 0.0 {
                    ...
                    if let Some(mem) = ecs.get_memory_mut(entity) { mem.mark_cell(t.cell_x, t.cell_y, CellTag::Food, 0.4); }
                } else {
