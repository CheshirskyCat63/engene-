...
impl ResourceGrid {
    pub fn new(biomes: &[Biome]) -> Self {
        ...
        Self { food, water, carcasses: Vec::new() }
    }
    ...
    pub fn tick(&mut self, delta: f32, biomes: &[Biome]) {
        ...
        self.carcasses.retain(|c| c.decay_timer > 0.0 && c.food_remaining > 0.01);
    }

    pub fn best_food_cell(&self, near_cx: u32, near_cy: u32, search_radius: u32) -> Option<(u32, u32)> {
    ...
    pub fn best_water_cell(&self, near_cx: u32, near_cy: u32, search_radius: u32) -> Option<(u32, u32)> {
        let mut best: Option<(u32, u32, f32)> = None;
        let r = search_radius as i32;
        for dy in -r..=r {
            for dx in -r..=r {
