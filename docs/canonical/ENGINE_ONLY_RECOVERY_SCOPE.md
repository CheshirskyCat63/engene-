...
## 🚨 QUARANTINE ORDER - IMMEDIATE

**All consumer/tooling crates are now QUARANTINED from engine recovery.**
...
# cargo check -p sdk_app          # ❌ QUARANTINED
# cargo check -p game_framework    # ❌ QUARANTINED  
# cargo check -p engine_tools      # ❌ QUARANTINED
# cargo test -p engene_sdk       # ❌ QUARANTINED
```
...
### Tier 2 - Quarantined (Consumer Layer)
```
...
### 1. NO FIXES FOR QUARANTINED CRATES
...
"QUARANTINED during engine-only recovery. Not part of current scope."
