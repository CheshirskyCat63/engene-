...
fn bench_spatial_index(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("query_radius_50", size), size, |b, _| {
            b.iter(|| {
                black_box(level.query_radius(500.0, 500.0, 50.0))
            });
        });
        
        group.bench_with_input(BenchmarkId::new("query_radius_200", size), size, |b, _| {
            b.iter(|| {
                black_box(level.query_radius(500.0, 500.0, 200.0))
            });
        });
    }
...
fn bench_ecs_component_access(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000].iter() {
        ...
        for i in 0..*size {
            let e = Entity { id: i as u64, generation: 0 };
            ecs.alive.push(e);
            ecs.transforms.insert(e, Transform { x: i as f32, y: 0.0, z: 0.0 });
            ecs.velocities.insert(e, Velocity { vx: 1.0, vy: 0.0, vz: 0.0 });
        }
        ...
        group.bench_with_input(BenchmarkId::new("read_write_iteration", size), size, |b, _| {
            b.iter(|| {
                for &e in &ecs.alive {
                    if let Some(t) = ecs.transforms.get(&e) {
...
fn bench_event_bus(c: &mut Criterion) {
    ...
    impl<T> EventBus<T> {
        ...
        fn emit(&mut self, event: T) { self.queue.push_back(event); }
    ...
    for count in [10, 100, 1000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("emit_and_drain", count), count, |b, &count| {
            let mut bus = EventBus::new();
            b.iter(|| {
                for i in 0..count {
                    bus.emit(TestEvent { value: i });
...
fn bench_ai_scheduler(c: &mut Criterion) {
    ...
    enum AiPriority { Idle = 0, Normal = 1, Urgent = 2, Critical = 3 }
    ...
    impl AiScheduler {
        fn new() -> Self {
            Self { schedules: HashMap::new(), frame: 0, budget_us: 4000 }
        }
        
        fn update_entity(&mut self, entity: Entity, priority: AiPriority) {
            self.schedules.entry(entity).and_modify(|s| s.priority = priority)
                .or_insert(EntitySchedule { last_tick_frame: 0, skip_count: 0, priority });
        }
        
        fn should_tick(&mut self, entity: Entity, elapsed_us: u64) -> bool {
            let schedule = self.schedules.get_mut(&entity);
            let schedule = match schedule { Some(s) => s, None => return true };
