...
impl WorldStreamer {
    ...
    fn load_chunk(&mut self, coord: ChunkCoord) -> Result<(), String> {
        self.loaded_chunks.insert(coord);
        Ok(())
    }
    
    fn unload_chunk(&mut self, coord: ChunkCoord) -> Result<(), String> {
        self.loaded_chunks.remove(&coord);
        Ok(())
    }
    
    fn is_chunk_loaded(&self, coord: ChunkCoord) -> bool {
        self.loaded_chunks.contains(&coord)
    }
    
    fn memory_usage(&self) -> usize {
        self.loaded_chunks.len() * 1_000_000 // 1MB per chunk
    }
}
...
impl ChunkPersistenceService {
    ...
    fn save_chunk(&mut self, coord: ChunkCoord, data: &[u8]) -> Result<(), String> {
        self.chunks.insert(coord, data.to_vec());
        Ok(())
    }
    
    fn load_chunk(&self, coord: ChunkCoord) -> Result<Vec<u8>, String> {
        self.chunks.get(&coord).cloned().ok_or_else(|| "Chunk not found".to_string())
    }
    
    fn save_chunk_compressed(&mut self, coord: ChunkCoord, data: &[u8]) -> Result<(), String> {
        // Mock compression
        self.save_chunk(coord, data)
    }
    
    fn load_chunk_compressed(&self, coord: ChunkCoord) -> Result<Vec<u8>, String> {
        // Mock decompression
        self.load_chunk(coord)
    }
    
    fn simulate_corruption(&mut self, coord: ChunkCoord) {
        self.chunks.remove(&coord);
    }
    
    fn recover_chunk(&mut self, coord: ChunkCoord) -> Result<(), String> {
        // Mock recovery
        Ok(())
    }
    
    fn save_chunk_with_version(&mut self, coord: ChunkCoord, data: &[u8], version: u32) -> Result<(), String> {
        let mut versioned_data = vec![version];
        versioned_data.extend_from_slice(data);
        self.chunks.insert(coord, versioned_data);
        Ok(())
    }
    
    fn load_chunk_with_version(&self, coord: ChunkCoord, expected_version: u32) -> Result<Vec<u8>, String> {
        let data = self.chunks.get(&coord).ok_or("Chunk not found")?;
        if data.is_empty() {
        ...
        if version != expected_version {
            return Err(format!("Version mismatch: expected {}, got {}", expected_version, version));
        }
        
        Ok(data[1..].to_vec())
    }
    
    fn save_chunk_entities(&mut self, coord: ChunkCoord, ecs: &Ecs) -> Result<(), String> {
        // Mock entity serialization
        let entity_count = ecs.alive.len();
        let data = vec![entity_count as u8];
        self.save_chunk(coord, &data)
    }
    
    fn load_chunk_entities(&self, coord: ChunkCoord, ecs: &mut Ecs) -> Result<(), String> {
        let data = self.load_chunk(coord)?;
        let entity_count = data[0] as usize;
        
        // Mock entity deserialization
    ...
    fn memory_usage(&self) -> usize {
        self.chunks.values().map(|data| data.len()).sum()
    }
}
