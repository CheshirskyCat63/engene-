...
fn ci_watches_main_and_transition_branch() {
    let workflow =
        fs::read_to_string(WORKFLOW_PATH).expect(".github/workflows/rust.yml must exist");

    let branches = extract_push_branches(&workflow);

    assert!(
        branches.contains(&"main".to_string()),
        "CI must watch 'main' branch, found: {:?}",
        branches
    );
    assert!(
        branches.contains(&"engene-2.0-transition".to_string()),
        "CI must watch 'engene-2.0-transition' branch, found: {:?}",
        branches
    );
}
...
fn ci_includes_fmt_gate() {
    ...
    assert!(
        jobs.contains(&"fmt".to_string()),
        "CI must have 'fmt' job, found: {:?}",
        jobs
    );
    ...
    assert!(
        fmt_section.contains("cargo fmt") && fmt_section.contains("--check"),
...
fn ci_includes_clippy_gate() {
    ...
    assert!(
        jobs.contains(&"clippy".to_string()),
        "CI must have 'clippy' job, found: {:?}",
        jobs
    );
    ...
    assert!(
        clippy_section.contains("clippy"),
        "clippy job must run clippy"
    );
    assert!(
        clippy_section.contains("-D warnings") || clippy_section.contains("-Dwarnings"),
...
fn ci_runs_smoke_lane() {
    ...
    assert!(
        jobs.contains(&"smoke".to_string()),
        "CI must have 'smoke' job, found: {:?}",
        jobs
    );
    ...
    assert!(
        smoke_section.contains("nextest"),
        "smoke job must use nextest"
    );
    assert!(
        smoke_section.contains("engine_contracts")
            && smoke_section.contains("production_candidate"),
...
fn ci_runs_contract_lane() {
    ...
    assert!(
        jobs.contains(&"contracts".to_string()),
        "CI must have 'contracts' job, found: {:?}",
        jobs
    );
    ...
    assert!(
        contracts_section.contains("nextest"),
...
fn ci_does_not_run_certification_or_perf_in_default_pr_path() {
    ...
    if has_certification || has_perf {
        ...
        assert!(
            !build_section.contains("certification") && !build_section.contains("perf"),
    ...
    for job in &required_jobs {
        assert!(
            jobs.contains(&job.to_string()),
            "CI must have '{}' job as required gate",
            job
        );
    }
}
