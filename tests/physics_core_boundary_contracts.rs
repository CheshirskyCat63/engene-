...
fn physics_runtime_owner_is_declared_as_engine_physics() {
    let workspace_cargo =
        fs::read_to_string("Cargo.toml").expect("Workspace Cargo.toml must exist");

    let members = extract_workspace_members(&workspace_cargo);

    // Verify engine_physics is in workspace
    assert!(
        members.iter().any(|m| m.contains("engine_physics")),
        "engine_physics must be declared in workspace members, found: {:?}",
        members
    );

    // Verify path points to crates/engine_physics
    let engine_physics_member = members
        .iter()
        .find(|m| m.contains("engine_physics"))
        .expect("engine_physics member must exist");

    assert!(
        engine_physics_member.contains("crates/engine_physics")
            || engine_physics_member.contains("engine_physics"),
        "engine_physics must be in crates/engine_physics, found: {}",
        engine_physics_member
    );
}
...
fn physics_is_capability_not_runtime_role() {
    ...
    assert!(
        feature_policy.to_lowercase().contains("physics")
    ...
    if role_section.contains("physics") {
        ...
        assert!(
            !role_section.contains("role_physics"),
...
fn root_crate_policy_does_not_allow_new_physics_ownership() {
    ...
    assert!(
        root_policy.to_lowercase().contains("new domain ownership")
...
fn physics_boundary_has_explicit_forbidden_dependencies() {
    ...
    assert!(
        boundary_doc.to_lowercase().contains("forbidden")
    ...
    assert!(
        boundary_doc.to_lowercase().contains("core")
            && boundary_doc.to_lowercase().contains("physics"),
        "PHYSICS_CORE_BOUNDARY must mention core and physics relationship"
    );
    assert!(
        boundary_doc.to_lowercase().contains("physics -> core")
...
fn physics_core_boundary_doc_exists_and_is_not_placeholder() {
    ...
    assert!(
        boundary_doc.contains("What is core"),
        "Boundary doc must describe core"
    );
    assert!(
        boundary_doc.contains("What is physics runtime"),
        "Boundary doc must describe physics runtime"
    );
    assert!(
        boundary_doc.contains("Where physics must live"),
        "Boundary doc must describe ownership"
    );
    assert!(
        boundary_doc.contains("What physics can read from core"),
        "Boundary doc must describe allowed dependencies"
    );
    assert!(
        boundary_doc.contains("What physics must NOT pull back to core"),
        "Boundary doc must describe forbidden dependencies"
    );
...
fn engine_physics_depends_on_core_not_reverse() {
    ...
    assert!(
        physics_has_core || physics_deps.is_empty(),
...
fn workspace_ownership_map_assigns_physics_to_engine_physics() {
    ...
    let has_physics_ownership = ownership_map.to_lowercase().contains("physics")
        && (ownership_map.contains("engine_physics") || ownership_map.contains("engine_physics"));
    assert!(
        has_physics_ownership,
