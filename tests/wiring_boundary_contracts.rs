...
fn ballistics_tick_descriptor_matches_boundary_contract() {
    ...
    // Verify name
    assert_eq!(
        desc.name, "BallisticsTick",
        "System name must be BallisticsTick"
    );
...
fn damage_dispatch_descriptor_matches_boundary_contract() {
    ...
    // Verify name
    assert_eq!(
        desc.name, "DamageDispatch",
        "System name must be DamageDispatch"
    );

    // Verify reads events (ImpactEvent)
    assert!(
        !desc.reads_events.is_empty(),
...
fn destruction_nav_occlusion_chain_uses_expected_events_only() {
    ...
    assert!(
        !destruction_desc.reads_events.is_empty(),
        "DestructionTickSystem must read events"
    );
    assert!(
        !destruction_desc.emits_events.is_empty(),
    ...
    assert!(
        !occlusion_desc.reads_events.is_empty(),
...
fn ai_descriptor_does_not_require_animation_descriptor() {
    ...
    assert!(
        ai_resource_count > 0 || anim_resource_count > 0,
...
fn gore_boundary_is_optional_at_descriptor_level() {
    ...
    assert!(
        !damage_desc.reads_events.is_empty(),
        "DamageDispatchSystem should read events"
    );
    assert!(
        !damage_desc.emits_events.is_empty(),
    ...
    assert!(
        damage_desc.emits_events.len() > 0,
        "DamageDispatchSystem should emit events"
    );
}
